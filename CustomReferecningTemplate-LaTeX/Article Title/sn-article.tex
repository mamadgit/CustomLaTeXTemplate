...
To tackle this issue, a significant amount of research, as cited in \cite{DBLP:conf/cav/LomuscioQR09, DBLP:phd/ethos/Raimondi06, DBLP:journals/entcs/RaimondiL04, DBLP:conf/icmas/Wooldridge00}, has utilised another type of semantics known as interpreted systems. This semantics is based on the idea of possible worlds, where it considers the global states in conjunction with the agents' local (internal) states. This can be seen as an improvement to the possible worlds semantics as it indexes the agents' knowledge states with their local states. While this approach does technically ground the agents' knowledge states in computational notions, according to a cited research, this method is a "very roundabout way of defining an agent's knowledge" \cite[5]{DBLP:conf/mallow/AlechinaL10}, meaning interpreted systems may not actually be computationally grounded. \par
Nevertheless, the current overall research landscape in reasoning about agent knowledge is the property of computationally grounded semantics is fixed, meaning if specific criteria are met in the design of a given semantics, it can be considered computationally grounded. However, just as how interpreted systems is regarded as an improvement to the possible worlds semantics due to its detail in indexing agents' accessible knowledge states, there are other semantical structures that take the level of indexing states even further  (e.g., vector-based interpreted systems, which will be elaborated further) or have entirely different structures to computationally ground agents' reasoning (e.g., stochastic or concurrent game structures). Therefore, it can be concluded that the property of computationally grounded semantics is not limited to specific criteria but is rather degree-based or gradational.\par
This is a critical perspective to consider in designing semantics for reasoning about agent behaviour. The gradational property of computationally grounded semantics suggests that the level of computational details in designing a semantical model, should be appropriate for the environment in which it is being used. This means in some environments, it is even possible the design of a semantical structure needs to be more abstract and flexible; designs closer to BDI frameworks and possible worlds semantics as they do not confine agents' accessible knowledge states to predefined situations through indexing agents' internal knowledge states and can, therefore, allow them to better adapt to unpredictable changes, maintaining their autonomy (a key characteristic of agents in a multi-agent system). \par 
For example, the importance of the mentioned perspective can be seen in concurrent game structures where the agents are resource-bounded. The inclusion of resources (a quantified parameter) presents the semantics with its computational properties and its overall natural design inherently avoids modelling the agents as omniscient in a highly efficient manner (more effectively than interpreted systems)\footnote{Concurrent game structures are computationally grounded in a different sense than interpreted systems.}. However, because the agents of the system perform actions in a combined strategic manner, each individual agent has limited knowledge and resources, and an action is only initiated when the combined resources of all agents satisfy the action's resource requirements. This makes it very difficult to keep track of the reasoning process of any singular agent. In other words, agents cannot explain their decision-making process. \par
In addition to elaborating further on the mentioned perspective through a detailed comparison of known semantics, this paper will offer new insight regarding semantical models for reasoning about agent knowledge, where the resource-bounded property of agents from concurrent game structures (which gives them its computational properties) are combined with interpreted systems. This structure can serve as a basis for new semantical models for improvement.
